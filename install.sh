...
aliasInstall() {
    ...
    if [[ -f "$HOME/install.sh" ]] && [[ -d "/etc/v2ray-agent" ]] && grep <"$HOME/install.sh" -q "作者:mack-a"; then
        ...
        if [[ -d "/usr/bin/" ]]; then
            if [[ ! -f "/usr/bin/" ]]; then
                ln -s /etc/v2ray-agent/install.sh /usr/bin/
                chmod 700 /usr/bin/
                Type=true
            fi
            ...
            if [[ ! -f "/usr/sbin/" ]]; then
                ln -s /etc/v2ray-agent/install.sh /usr/sbin/
                chmod 700 /usr/sbin/
                Type=true
            fi
            rm -rf "$HOME/install.sh"
        fi
        if [[ "${fanqiangType}" == "true" ]]; then
            echoContent green "快捷方式创建成功，可执行[fanqiang]重新打开脚本"
        fi
    fi
}
